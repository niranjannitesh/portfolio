...
Card.Description = function CardDescription({ children }: any) {
  return (
    <p className="relative z-10 mt-2 text-sm text-[#676767] leading-7">
      {children}
    </p>
  )
}
...
Card.Cta = function CardCta({ children }: any) {
  return (
    <div
      aria-hidden="true"
      className="relative z-10 flex items-center mt-4 text-sm font-medium text-rose-700"
    >
      {children}
...
Card.Eyebrow = function CardEyebrow({
  ...
  return (
    <Component
      className={clsx(
        className,
        "relative z-10 order-first mb-3 flex items-center text-sm text-[#676767]",
        decorate && "pl-3.5"
      )}
      {...props}
    >
